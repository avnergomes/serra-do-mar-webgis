...
- **Trilhas oficiais x não verificadas**: a tabela de atributos das trilhas ganhou a coluna **`oficial`** (`sim`/`nd`) mais `oficial_fonte`, preenchida por `mark_official_trails.py` só onde o nome bate com uma trilha reconhecida no plano do parque (Pico Paraná: trilhas consolidadas do Plano de Uso Público, Portaria IAT 470/2025; Marumbi: trilhas sinalizadas branca/vermelha/azul/amarela + Circuito + Itupava; Baitaca: Anhangava, Pão de Loth, Samambaia e Campo de Asa Delta). As **34 trilhas oficiais** são desenhadas em destaque (linha cheia, grossa) com selo "✓ Trilha oficial" no popup; as demais ficam com **ênfase reduzida** (linha fina, pontilhada, translúcida). Critério conservador e editável no script.
